...
class SmartTrader(object):
    def __init__(self, step, input_size, starter_learning_rate, hidden_size, nclasses, decay_step=500, decay_rate=1.0,
                 cost=0.0002):
        '''
...
def main(operation='train', code=None):
    ...
    if operation == 'train':
        ...
        # train_set = DataSet(moving_features[:-validation_size], moving_labels[:-validation_size])
        val_set = DataSet(moving_features[-validation_size:], moving_labels[-validation_size:])
        predict(val_set, step=step, input_size=input_size, learning_rate=learning_rate, hidden_size=hidden_size,
                nclasses=nclasses)
    else:
